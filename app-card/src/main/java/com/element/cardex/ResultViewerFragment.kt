...
import com.element.camera.ElementOcrService
...
class ResultViewerFragment(
        ...
        private val docTypeId: String? = null
) : DialogFragment() {

    private val revision = mutableMapOf<String, String>()
    private val receiver = ElementServiceResultReceiver { _, _ ->
        ...
        return inflater.inflate(R.layout.fragment_result, container, false).apply {
            when (resultCode) {
                ...
                OCR_RESULT_OK -> {
                    ...
                    result.getParcelableArrayList<ElementOcrResultField>(KEY_OCR_RESULTS)?.let { ocrResult ->
                        box.addView(inflateResultForm(inflater, ocrResult))
                    }
                }
            }
            findViewById<Button>(R.id.revision).apply {
                if (userId != null && docTypeId != null) {
                    text = getString(R.string.revision)
                    setOnClickListener {
                        ElementOcrService.startOcrRevisionService(context, receiver, userId, docTypeId, revision)
                    }
                } else {
    ...
    private fun inflateResultForm(inflater: LayoutInflater, data: List<ElementOcrResultField>, @StringRes title: Int = R.string.ocr_result_title) =
