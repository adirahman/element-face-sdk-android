...
# Element Face SDK
The Element Face SDK (the SDK) is an API library for creating biometrics models that can be used to authenticate users. This document contains information to integrate the SDK into an Android application by using Android Studio.

The SDK uses a modularized architecture to achieve flexible project designs and to fulfill various requirements. The four main components of the SDK are:
...
### Dependencies for `element-face-cam` & `element-face-core`
...
References of dependencies can be found in the sample projects at [app-mobile-enrollment](../app-mobile-enrollment/build.gradle) and [app-server-authentication](../app-server-authentication/build.gradle).
...
1. These permissions are declared in the Element Face SDK AAR, and your app will inherit them. No need to declare them again in your app.
1. Use `PermissionUtils.verifyPermissions(Activity activity, String... permissionsToVerify)` provided by the SDK to ask for user permissions:
    ...
    ```
1. For Android 6.0 (Marshmallow, API 23) and up, make sure the permissions are granted before starting any Activity provided by the Element Face SDK.

### SDK Debug Mode
The debug mode can be enabled with the Element FM SDK. It is disabled by default. Once the debug mode is ON, all image captures will be sent to Element for investigation. Please use this feature if you need help from Element.
```
  ElementFaceSDK.enableDebugMode(getBaseContext(), true);
```

### Element Activities
The SDK provides various Activities, `ElementFaceAuthActivity`, `ElementFaceCaptureActivity`, and `ElementFaceEnrollActivity`. Usage of each Activity will be covered in the next section.
...
- `EXTRA_ELEMENT_USER_ID` - A `UserInfo` needs to be created first before starting the Element Activities. Pass the `userId` of it with the intent extra.
- `EXTRA_LIVENESS_DETECTION` - Enable face liveness detection. It's effective only if `element-face-ui` is installed. The default value is `false`.
- `EXTRA_TUTORIAL` - Display tutorials to guide the user step-by-step. It's also a feature from 'element-face-ui'. The default value is `false`.
- `EXTRA_SECONDARY_TUTORIAL` - Display small instructional popups during/after face detection. The default value is `false`.

## Usage of the Element Activities
### User face matching on server
The `ElementFaceCaptureActivity` allows capturing selfies for further processing on the server side.
...
1. Will send the captured image data to the server if the `resultCode` is OK
    ```
      public class FmActivity extends ElementFaceCaptureActivity {

        @Override
        public void onImageCaptured(@Nullable Capture[] captures, @NonNull String resultCode) {
            if (CAPTURE_STATUS_VALID_CAPTURES.equals(resultCode)) {
                // CAPTURE_STATUS_VALID_CAPTURES is returned when frames are captured but before processing. You may update UI here.
                .....
            } else if (CAPTURE_RESULT_OK.equals(resultCode) || CAPTURE_RESULT_GAZE_OK.equals(resultCode)) {
